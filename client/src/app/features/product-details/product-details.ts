...
export class ProductDetails implements OnInit {
  ...
  giveFeedback(productId: number){
    if(this.comment === "" || this.rating == 0){
        this.toast.error("Please give rating and comment to give the feedback")
        return;
    }
  ...
  // fetchReview(productId: number){
  //    this.productSer.getReview(productId).pipe(takeUntilDestroyed(this.destroyRef))
  //                   .subscribe({
  //                       next: (res)=>{
